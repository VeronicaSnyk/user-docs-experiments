...
# Resilient .env discovery — works whether this skill lives under .docs-agent/skills
# (local) or .claude/skills (CI copy). Checks the docs-agent dir, the skill dir, then cwd.
def _load_env():
    script_dir = Path(__file__).resolve().parent
    for cand in (
        script_dir.parents[2] / ".env",   # .../.docs-agent/.env or .../.claude/.env
        script_dir.parents[1] / ".env",   # skill dir /.env
        Path.cwd() / ".docs-agent" / ".env",
        Path.cwd() / ".env",
    ):
        if cand.is_file():
            load_dotenv(cand); return

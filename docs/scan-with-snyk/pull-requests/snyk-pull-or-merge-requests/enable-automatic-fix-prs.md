...
* If you click **Retest now** for a Project, a manual scan runs. The 24-hour window is marked as having the scan run, and no automatic PR is created until the next automated scan.
...
3. In the **Automatic Fix PRs** section, check **New vulnerabilities**.
4. Click **Save.**
...
If you check **Fix all vulnerabilities for the same dependency in a single PR**, this adds any PR opened to address an issue with any potential PR for upgrades that also fix the same issue. Fixing an instance of the vulnerability often fixes other vulnerabilities as well.

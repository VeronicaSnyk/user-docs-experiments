...
### Step -1: Label pre-check (run before anything else)

Fetch just enough of the ticket to read its labels. Exit immediately if no docs work is needed.

1. Inspect the ticket labels.
2. If the **only** ship-it label present is `ship-it-release` (with or without the bare `ship-it` label), and there is **no** docs signal in the summary, description, or comments (no docs label, no Confluence/spec/PRD link, no explicit mention of documentation):
   - Stop with: "No docs needed — ticket is release-tracking only (`ship-it-release`). Nothing to draft."
   - Do not proceed to Step 0 or any later step.
3. If the ticket carries `ship-it-new-docs`, or has a docs-related link or explicit docs request in the description/comments, continue to Step 0.
